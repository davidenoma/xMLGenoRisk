...
def draw_roc_curve(x, y, cv, indices_new, title='ROC Curve'):
    ...
    for i, (train_index, test_index) in enumerate(cv.split(x, y)):
        ...
        counter = counter + 1
            ...
            # Train
        pred_proba = all_results_SVM(X_train, Y_train, X_train, Y_train, indices_new)
...
def draw_roc_curve_dev(x, y, cv, indices_new, title='ROC Curve for Dev Set'):
    ...
    for i, (train_index, test_index) in enumerate(cv.split(x, y)):
        ...
        # if (len(indices_new[counter])):
            # Dev
        pred_proba = all_results_SVM(X_train, Y_train, X_test, Y_test, indices_new)
...
def draw_roc_curve_Test(x, y, cv, indices_new, title='Test ROC Curve'):
    ...
    for i, (train_index, test_index) in enumerate(cv.split(x, y)):
        ...
        # if (len(indices_new[counter])):
            # Train
        pred_proba = all_results_SVM(X_train, Y_train, x_cv, y_cv, indices_new)
